...
int test3( )
{
    ...
    cout << "v, sum: " << v.sum() << ", length: " << v.length() << ", max:" << v.maximum() << ", min:" << v.minimum() << endl;
    v = t + s;
    if (!(t == v))
    {       
        return 0;
...
int test4( )
{
    ...
    if (!close(s.sum( ), 80)) {cout << 4 << endl; return 0;}
    if (s.minimum( ) != 0) {cout << 5 << endl; return 0;}
    if (s.maximum( ) != 40){cout << 6 << endl; return 0;}
    if (!close(s.mean( ), 80.0/4)) {cout << 7 << endl; return 0;}

    s = -2*u;
    if (s.length( ) != 4) {cout << 8 << endl;return 0;}
    if (!close(s.sum( ), -80)) {cout << 9 << endl; return 0;}
    if (s.minimum( ) != -40) {cout << 10 << endl; return 0;}
    if (s.maximum( ) != 0) { cout << 11 << endl; return 0;}
    if (!close(s.mean( ), -80.0/4)) {cout << 12 << endl; return 0; }

    s = 0*u;
    if (s.length( ) != 4) {cout << 13 << endl; return 0;}
    if (!close(s.sum( ), 0)) {cout << 14 << endl; return 0;}
    if (s.minimum( ) != 0) {cout << 15 << endl; return 0;}
    if (s.maximum( ) != 0) {cout << 16 << endl; return 0;}
    if (!close(s.mean( ), 0)) {cout << 17 << endl; return 0;}

    s = 10 * t;
    if (s.length( ) != 0) {cout << 18 << endl; return 0;}
    if (s.sum( ) != 0) {cout << 19 << endl;return 0;}
